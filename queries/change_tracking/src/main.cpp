...
int main(int, char *[]) {
  ...
  // query is iterated.
  flecs::query<const 位置> q_read =
      ecs.query_builder<const 位置>().detect_changes().build();

  // Create a query that writes the component based on a Dirty state.
  flecs::query<const Dirty, 位置> q_write =
      ecs.query_builder<const Dirty, 位置>()
          .term_at(0)
          .up(flecs::IsA) // Only match Dirty from prefab
          .build();
  ...
  ecs.entity("e1").is_a(p1).set<位置>({10, 20});

  ecs.entity("e2").is_a(p1).set<位置>({30, 40});

  ecs.entity("e3").is_a(p2).set<位置>({40, 50});

  ecs.entity("e4").is_a(p2).set<位置>({60, 70});
