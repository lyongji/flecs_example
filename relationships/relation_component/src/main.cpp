...
// This example shows how relationships can be combined with components to
// attach data to a relationship.
...
int main(int, char *[]) {
  ...
  // the first element:
  flecs::entity e3 = ecs.entity().set<Expires, 位置>({0.5});
  const Expires &e = e3.get<Expires, 位置>();
  ...
  std::cout << ecs.pair<Expires, 位置>().type_id().path() << "\n";
  std::cout << ecs.pair<MustHave, 位置>().type_id().path() << "\n";

  // When querying for a relationship component, add the pair type as template
  // argument to the builder:
  flecs::query<Requires> q =
      ecs.query_builder<Requires>()
          .term_at(0)
          .second<Gigawatts>() // set second part of pair for first term
          .build();
