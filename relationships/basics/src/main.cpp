...
int main(int, char *[]) {
  ...
  // Create an entity with 3 relationships. Relationships are like regular
  // components, but combine two types/identifiers into an (relationship,
  // object) pair.
  ...
  std::cout << "Bob eats apples? " << bob.has<吃>(apples) << "\n";
  ...
  bob.each<吃>([](flecs::entity second) {
  ...
  std::cout << "Bob eats " << bob.target<吃>().name() << "\n";

  // Get second target of relationship
  std::cout << "Bob also eats " << bob.target<吃>(1).name() << "\n";
}
