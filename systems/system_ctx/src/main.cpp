...
double distance_sqr(const 位置 &p1, const 位置 &p2) {
  return sqr(p2.x - p1.x) + sqr(p2.y - p1.y);
}

double randd(int max) { return static_cast<double>((rand() % max)); }

using CollisionQuery = flecs::query<const 位置, const Radius>;

int main(int, char *[]) {
  flecs::world ecs;

  CollisionQuery q_collide = ecs.query<const 位置, const Radius>();

  flecs::system sys =
      ecs.system<const 位置, const Radius>("Collide")
          .ctx(&q_collide)
          .each([](flecs::iter &it, size_t i, const 位置 &p1,
                   const Radius &r1) {
            CollisionQuery *q = it.ctx<CollisionQuery>();
            flecs::entity e1 = it.entity(i);

            q->each([&](flecs::entity e2, const 位置 &p2, const Radius &r2) {
              if (e1 == e2) {
