...
int main(int, char *[]) {
  ...
  ecs.observer<位置, 速度>()
      ...
      .each([](flecs::iter &it, size_t i, 位置 &, 速度 &) {
        if (it.event() == flecs::OnAdd) {
          std::cout << " - Enter: " << it.event_id().str() << ": "
                    << it.entity(i).name() << "\n";
        } else if (it.event() == flecs::OnRemove) {
          std::cout << " - Leave: " << it.event_id().str() << ": "
                    << it.entity(i).name() << "\n";
        }
      });
  ...
  // This triggers the monitor with EcsOnRemove, as the entity no longer
  // matches.
  e.remove<位置>();
